...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}p{1.9cm}>{\centering\arraybackslash}p{1.6cm}>{\centering\arraybackslash}p{1.6cm}>{\centering\arraybackslash}p{2.0cm}X@{}}
        ...
        Joint GRPO & 600/600 & 10.72 & \makecell[c]{tail-10 reward\\2.4016} & 奖励权重为 \((1.0,1.0,1.0)\)，后段奖励保持稳定，导出可评测权重。 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}p{2.6cm}p{4.1cm}X@{}}
        ...
        \makecell[l]{\texttt{evaluate}\\\texttt{\_transfer.py}} & 输入 Base/GRPO 权重与 benchmark 场景；输出评测结果与增益表 & 触发 Stage 3/4 评测并生成优化模型对比结果。 \\
        \bottomrule
    \end{tabularx}
\end{table}
