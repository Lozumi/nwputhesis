...
为补充正文第3章对阶段2、阶段3和阶段4实现过程的说明，本节给出数据构建 Web 工作台中的代表性界面截图。该工作台用于人工复核、任务生成、数据浏览和指标检查等交互环节；批量生成与导出仍由命令行工具链统一调度。界面截图主要说明各阶段的可视化复核与质量控制方式，而不作为实验结果数据来源。
...
\autoref{fig:app-stage4-task-generation} 展示了阶段4图像任务生成界面。该界面按场景、任务类型、难度、地标类别和采样数量组织图像 QA 样本生成，右侧同时给出当前场景可用地标、任务可用性和最新实验摘要。该设计便于在生成前检查地标是否支持四类图像任务，在生成后继续切换到数据浏览、实验、结果和指标页面，核对参考图像、查询图像、候选选项和目标框监督是否一致。
...
第4章正文保留能力优化方法、训练流程概述和任务级评测结果。本附录补充训练环境、运行记录、实现入口、训练流程伪代码、完整性核查逻辑和奖励函数关键片段，用于说明方法已经落实为可执行训练链路。
...
正式训练结束后，系统对运行目录进行完整性核查，重点确认数据快照、训练计划、日志文件、指标文件、checkpoint、LoRA adapter、合并模型和评测入口是否完整。该核查不用于证明能力提升，而用于保证第4章报告的训练过程能够追溯到明确的数据版本、参数配置和模型产物。
